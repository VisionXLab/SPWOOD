...
class Semi_GmmLossforLabeledData9(nn.Module):
    def __init__(self, 
        ...
        super(Semi_GmmLossforLabeledData9, self).__init__()
    ...
    def loss_single(self, t_logits_list, s_logits_list, batch_gt_instances_from_teacher, level_inds, ratio=0.03, img_metas=None, bbox_head=None, featmap_sizes=None):
        ...
        labels, bbox_targets, angle_targets, bid_targets, centerness_targets = bbox_head.get_targets(
            all_level_points, [batch_gt_instances_from_teacher], self.strides)
        ...
        with torch.no_grad():
            ...
            # 7. 最终的索引
            selected_inds = torch.cat([selected_prompt_indices, selected_indices, pos_inds], 0).unique()  # 确保索引有序
            # num_prompt_final_forlabeleddata = torch.tensor(selected_prompt_indices.size(0), device=selected_prompt_indices.device, dtype=torch.long)
            # num_final_forlabeleddata = torch.tensor(selected_inds.size(0), device=selected_inds.device, dtype=torch.long)
    ...
    def forward(self, teacher_logits, student_logits, ratio=0.03, img_metas=None, bbox_head=None, **kwargs):
        ...
        img_t_logits_list = self.pre_processing(teacher_logits[:4])
        img_s_logits_list = self.pre_processing(student_logits[:4])
