...
class Semi_GmmLoss9_Wo_P_Adpinds(nn.Module):
    def __init__(self, 
        ...
        super(Semi_GmmLoss9_Wo_P_Adpinds, self).__init__()
    ...
    def select_topk_gmm(self, start_idx, end_idx, use_centerness, teacher_probs, teacher_centernesses, mask, topk_num=2000):
        ...
        # NOTE 修改记录：针对第一次thres的分割结果进行调整
        selected_inds_fine_count = len(selected_inds_fine)
        layer_inds_count = len(layer_inds)
        ratio_pn = selected_inds_fine_count/(layer_inds_count - selected_inds_fine_count)  # GMM分割的正负比例
        # 比如切割的正负比是0.3，经过e运算之后的系数是0.74，多取一些样本
        thres_new = thres * math.exp(-ratio_pn)
        fine_mask = layer_vals > thres_new
        selected_inds_fine = layer_inds[fine_mask]
        ...
        # ===========以下分析是为了找出动态调整选点策略的突破口===========
        '''
        # --- 计算粗筛和细筛贡献比例的代码从这里开始 ---
        # 1. 检查 selected_indices_coarse 中的元素有多少在最终的 selected_indices 中
        isin_coarse = torch.isin(selected_indices_coarse, selected_indices)
        coarse_contributed_count = torch.sum(isin_coarse).item()
        # 2. 检查 selected_inds_fine 中的元素有多少在最终的 selected_indices 中
        isin_fine = torch.isin(selected_inds_fine, selected_indices)
        fine_contributed_count = torch.sum(isin_fine).item()
        # 3. 计算各自的贡献比例
        coarse_contribution_ratio = coarse_contributed_count / len(selected_indices_coarse)
        fine_contribution_ratio = fine_contributed_count / len(selected_inds_fine)
        
        selected_inds_coarse_count = len(selected_indices_coarse)
        selected_inds_fine_count = len(selected_inds_fine)
        layer_inds_count = len(layer_inds)
        ratio_pn = selected_inds_fine_count/(layer_inds_count - selected_inds_fine_count)  # GMM分割的正负比例
        # 将结果追加写入文件
        file_path = "/inspire/hdd/project/wuliqifa/gaoyubing-240108110053/zw/mcl/zw_result/ratio_2050.txt"
        with open(file_path, "a") as f:
            # f-string 格式化，确保每个数据后面都有换行符
            # topk数量  由topk的粗筛数量  由thres的细筛数量  thres分割的正负比例     
            f.write(f"{topk_num}\t{selected_inds_coarse_count}\t{coarse_contribution_ratio}\t{selected_inds_fine_count}\t{fine_contribution_ratio}\t{ratio_pn}\n")
            
        # ===========以上分析是为了找出动态调整选点策略的突破口===========
                
        # NOTE 结论：thres给出的P的贡献度为0.99+，因此需要给thres添加一个比例，筛选更多的P。
        # 直观理解：由于已经进入无监督阶段，基于thres分割结果的正负样本比例越大，说明模型预测到的正样本越多，一般来讲，GT的标注也就也多，这个时候减小thres，以筛选更多的正样本。  
        '''
