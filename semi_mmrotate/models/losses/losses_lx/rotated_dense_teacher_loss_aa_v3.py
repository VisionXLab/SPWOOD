...
class RotatedDTLossAssignerAssistentV3(nn.Module):
    def __init__(self, cls_channels=len(CLASSES), loss_type='origin', bbox_loss_type='l1', image_class_prompt_path='/mnt/nas-new/home/zhanggefan/liuxiang/mcl/Assinger_Assistent/image_class_prompt_from_chat.pt',
                angle_coder=dict(
                     type='PSCCoder',
                     angle_version='le90',
                     dual_freq=False,
                     num_step=3,
                     thr_mod=0)):
        super(RotatedDTLossAssignerAssistentV3, self).__init__()
    ...
    def convert_shape(self, logits):
        ...
        for angle_pred in angle_preds:
            B, C, H, W = angle_pred.shape
            angle_pred_flat = angle_pred.permute(0, 2, 3, 1).reshape(-1, C)  # → [B*H*W, 3]
            angle_decoded = self.angle_coder.decode(angle_pred_flat, keepdim=True).detach()  # → [B*H*W, 1]
            decode_angle_preds.append(angle_decoded.reshape(B, 1, H, W))
        #print(f'decode_angle_preds shape: {decode_angle_preds[0].shape}')  # (batch_size, h*w, 5)
