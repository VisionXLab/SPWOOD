...
class Semi_GmmLossforLabeledData6(nn.Module):
    def __init__(self, 
        ...
        super(Semi_GmmLossforLabeledData6, self).__init__()
    ...
    def loss_single(self, t_logits_list, s_logits_list, level_inds, ratio=0.03, img_metas=None, bbox_head=None, featmap_sizes=None):
        ...
        with torch.no_grad():
            ...
            #print(f"Threshold: {thres}, Selected Indices Count for labeled data: {len(selected_inds)}")
            ...
            # fine_inds for prompt
            joint_confidences_prompt = joint_confidences_all[prompt_inds]
            max_vals_prompt = torch.max(joint_confidences_prompt, 1)[0]
            self.mean_score.append(max_vals_prompt.mean())
            thres_prompt = self.gmm_policy(max_vals_prompt, policy = self.policy)
            self.thres.append(thres_prompt)
            selected_inds_prompt = torch.nonzero(max_vals_prompt > thres_prompt).squeeze(-1)
            selected_inds_prompt_global = prompt_inds[selected_inds_prompt]
            #print(f"Prompt Threshold: {thres_prompt}, Selected Indices Count for labeled data: {len(selected_inds_prompt_global)}")
            
            selected_inds_prompt, counts_prompt = torch.cat([selected_inds_prompt_global, prompt_inds], 0).unique(return_counts=True)
            selected_inds_prompt = selected_inds_prompt[counts_prompt>1]
            #print(f"Selected prompt Indices Count for labeled data: {len(selected_inds_prompt)}")
