...
class Semi_GmmLossforLabeledData7(nn.Module):
    def __init__(self, 
        ...
        super(Semi_GmmLossforLabeledData7, self).__init__()
    ...
    def loss_single(self, t_logits_list, s_logits_list, level_inds, ratio=0.03, img_metas=None, bbox_head=None, featmap_sizes=None):
        ...
        with torch.no_grad():
            ...
            # fine_inds
            self.mean_score = []
            self.mean_score.append(max_vals.mean())
            ...
            #print(f"Threshold: {thres}, Selected Indices Count: {len(selected_inds)}")
            ...
            # ------------------------------------
            # 新增：分层级的、针对Prompt类别的Top-K筛选
            # ------------------------------------
            
            # --- P3层 Prompt Top-K ---
            # 1. 隔离出P3层的数据
            max_inds_p3 = max_inds[:level_inds[0]]
            max_vals_p3 = max_vals[:level_inds[0]]
            
            # 2. 在P3层内部，找到所有符合Prompt条件的样本
            prompt_mask_p3 = torch.isin(max_inds_p3, exist_classes_tensor)
            prompt_vals_p3 = max_vals_p3[prompt_mask_p3]
            # 获取这些样本在P3内部的“本地索引”
            local_prompt_inds_p3 = torch.where(prompt_mask_p3)[0]
            
            # 3. 对P3层的Prompt候选进行Top-K筛选
            k_p3 = int(count_num * 0.03)
            if len(local_prompt_inds_p3) > k_p3:
                _, sorted_local_inds = torch.topk(prompt_vals_p3, k_p3)
                # 使用排序后的索引，从本地索引列表中选出Top-K
                selected_prompt_inds_p3 = local_prompt_inds_p3[sorted_local_inds]
            else:
                # 如果候选数不足，则全部选中
                selected_prompt_inds_p3 = local_prompt_inds_p3
            # P3的本地索引就是全局索引，无需转换

            # --- P4层 Prompt Top-K ---
            # 1. 隔离出P4层的数据
            start_p4, end_p4 = level_inds[0], level_inds[1]
            max_inds_p4 = max_inds[start_p4:end_p4]
            max_vals_p4 = max_vals[start_p4:end_p4]

            # 2. 在P4层内部，找到所有符合Prompt条件的样本
            prompt_mask_p4 = torch.isin(max_inds_p4, exist_classes_tensor)
            prompt_vals_p4 = max_vals_p4[prompt_mask_p4]
            local_prompt_inds_p4 = torch.where(prompt_mask_p4)[0]

            # 3. 对P4层的Prompt候选进行Top-K筛选
            k_p4 = int(count_num * 0.03) # P4层分配的名额
            if len(local_prompt_inds_p4) > k_p4:
                _, sorted_local_inds = torch.topk(prompt_vals_p4, k_p4)
                selected_local_inds_p4 = local_prompt_inds_p4[sorted_local_inds]
            else:
                selected_local_inds_p4 = local_prompt_inds_p4
            # 将P4的“本地索引”转换为“全局索引”
            selected_prompt_inds_p4 = selected_local_inds_p4 + start_p4

            # --- P5+层 Prompt Top-K ---
            # 1. 隔离出P5+层的数据
            start_rest = level_inds[1]
            max_inds_rest = max_inds[start_rest:]
            max_vals_rest = max_vals[start_rest:]

            # 2. 在P5+层内部，找到所有符合Prompt条件的样本
            prompt_mask_rest = torch.isin(max_inds_rest, exist_classes_tensor)
            prompt_vals_rest = max_vals_rest[prompt_mask_rest]
            prompt_inds_rest = torch.where(prompt_mask_rest)[0]

            
            # 3. 对P5+层的Prompt候选进行Top-K筛选
            k_rest = int(count_num * 0.05) # P5+层分配的名额
            if len(prompt_inds_rest) > k_rest:
                _, sorted_local_inds = torch.topk(prompt_vals_rest, k_rest)
                selected_local_inds_rest = prompt_inds_rest[sorted_local_inds]
            else:
                selected_local_inds_rest = prompt_inds_rest

            # 将P5+的“本地索引”转换为“全局索引”
            selected_prompt_inds_rest = selected_local_inds_rest + start_rest
            # --- 合并所有层级的Prompt Top-K结果 ---
            prompt_topk_inds = torch.cat([
                selected_prompt_inds_p3, 
                selected_prompt_inds_p4, 
                selected_prompt_inds_rest
            ])
            #prompt精筛
            ...
            if len(prompt_inds) > 0:
                self.mean_score.append(prompt_vals.mean())
                thres_prompt = self.gmm_policy(prompt_vals, policy=self.policy)
                self.thres.append(thres_prompt)
                selected_inds_prompt_global = torch.nonzero(prompt_vals > thres_prompt).squeeze(-1)
            else:
                selected_inds_prompt_global = torch.tensor([], device=t_cls_scores.device, dtype=torch.long)

            selected_inds_prompt, count_prompt = torch.cat([prompt_topk_inds, selected_inds_prompt_global], 0).unique(return_counts=True)
            selected_inds_prompt = selected_inds_prompt[count_prompt > 1]  # 保留重复的 Prompt 类别索引
            print(f"Prompt Selected Indices Count for labeled data: {len(selected_inds_prompt)}")
            ...
            class_topk_count = 1  # 每个非 Prompt 类别选取的数量
            class_topk_inds = []
            ...
            selected_inds = torch.cat([selected_inds_prompt, class_topk_inds, selected_inds, pos_inds], 0).unique().long()
            #print(f"Selected Indices Count: {len(selected_inds)}")
