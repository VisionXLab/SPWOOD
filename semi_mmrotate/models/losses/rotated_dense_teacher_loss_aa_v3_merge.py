...
class RotatedDTLossAssignerAssistentV3Merge(nn.Module):
    def __init__(self, cls_channels=len(CLASSES), loss_type='origin', bbox_loss_type='l1', image_class_prompt_path='/workspace/animax/MCL/tools/Assinger_Assistent/image_class_prompt_from_chat_with_percent5_label_modified.pt'):
        super(RotatedDTLossAssignerAssistentV3Merge, self).__init__()
    ...
    def loss_single(self, t_logits_list, s_logits_list, ratio, img_metas, level_inds):
        ...
        with torch.no_grad():
            ...
            # 去重  
            selected_inds_two = torch.cat([prompt_inds, overall_inds, class_topk_inds])
    ...
    def forward(self, teacher_logits, student_logits, ratio=0.03, img_metas=None, bbox_head=None, **kwargs):
        ...
        img_t_logits_list = self.pre_processing(teacher_logits, bbox_head.angle_coder)
        img_s_logits_list = self.pre_processing(student_logits, bbox_head.angle_coder)
