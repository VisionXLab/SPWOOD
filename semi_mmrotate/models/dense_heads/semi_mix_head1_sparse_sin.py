...
class SemiMixHead1SparseSin(RotatedAnchorFreeHead):
    ...
    def get_single_img_cls_loss_list(self, ):
    ...
    def loss(
        ...
        assert len(cls_scores) == len(bbox_preds) == len(angle_preds) == len(centernesses) # 列表长度为5
        ...
        # TODO 获得一个类别loss列表，分别对应每一张图片的类别loss
        # 这个等等写，先写一个简单的。对于全部的一起处理
        losses_list_p = multi_apply(
                    self.get_single_img_cls_loss_list,
                    pred_cls_scores = cls_scores,         # 5个元素的list 预测
                    pred_centernesses = centernesses,     # 5个元素的list 预测
                    phr_labels=labels,  # 5个元素的list 真实
                    all_level_points=all_level_points)
        
        losses_list_hr = multi_apply(
                    self.get_single_img_cls_loss_list,
                    pred_cls_scores = cls_scores,         # 5个元素的list 预测
                    pred_centernesses = centernesses,     # 5个元素的list 预测
                    phr_labels=hr_labels,  # 5个元素的list 真实
                    all_level_points=all_level_points)
        ...
        loss_cls = self.loss_cls(  
            flatten_cls_scores, flatten_labels, avg_factor=num_pos)  # flatten_cls_scores torch.Size([330088, 15])      flatten_labels.shape torch.Size([330088])
