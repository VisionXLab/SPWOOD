import torch
import numpy as np
from .rotated_semi_detector1 import RotatedSemiDetector1
...
class MCLTeacherRes(RotatedSemiDetector1):
    def __init__(self, model: dict, semi_loss_unsup, semi_loss_sup, train_cfg=None, test_cfg=None):
        super(MCLTeacherRes, self).__init__(
    ...
    def forward_train(self, imgs, img_metas, **kwargs):
        super(MCLTeacherRes, self).forward_train(imgs, img_metas, **kwargs)  # ???
        ...
        sup_losses = self.student.forward_train(**format_data['sup'], need_res=True)   # 在burn in阶段，加入辅助任务loss。有辅助loss
