...
class RotatedSemiDetector1(RotatedBaseDetector):
    def __init__(self, model: dict, semi_loss_unsup=None, semi_loss_sup=None, train_cfg=None, test_cfg=None):
        super(RotatedSemiDetector1, self).__init__()
        self.submodules = list(model.keys())
        for k, v in model.items():
            setattr(self, k, v)
        if semi_loss_unsup is not None:
            self.semi_loss_unsup = build_loss(semi_loss_unsup)        
        if semi_loss_sup is not None:
            self.semi_loss_sup = build_loss(semi_loss_sup)
